...
# class OrthoCase:
#     def __init__(self, oas_file_path):
#         self.oas_file_path = oas_file_path
#         self.doc = PyOrthoDoc()
#         self.doc.OpenCase(oas_file_path)
#         self.case = self.doc.getCase()
#         self.caseID = self.case.GetCaseID()
#         self.tp = self.doc.GetActiveTreatmentPlan()
#         self.short_file_path = self.oas_file_path.split("/")[-1]
    
#     def get_tooth_vertices_faces(self, tooth_clID: int) -> tuple:
#         """Return the vertices and faces of a tooth mesh"""
#         tooth = self.tp.getToothByID(tooth_clID) 
#         if tooth is None: # 
#             return None, None
#         mesh = tooth.getToothSurface()
#         vertices = mesh.vertices()
#         faces = mesh.faces()
#         faces = [i for face in faces for i in face]  # get flat faces
#         complete_vertices = np.array([vertices[i]
#                                      for i in faces], dtype=np.float32)
#         return complete_vertices, faces

#     def get_crown_vertices_faces(self, tooth_clID: int) -> tuple:
#         """Return the vertices and faces of a crown mesh"""
#         tooth = self.tp.getToothByID(tooth_clID) 
#         if tooth is None: # 
#             return None, None
#         mesh = tooth.getCrownData()
#         vertices = mesh.vertices()
#         faces = mesh.faces()
#         faces = [i for face in faces for i in face]  # get flat faces
#         complete_vertices = np.array([vertices[i]
...
#     def get_root_vertices_faces(self, tooth_clID: int) -> tuple:
#         """Return the vertices and faces of a root mesh"""
#         tooth = self.tp.getToothByID(tooth_clID) 
#         if tooth is None: # 
#             return None, None
#         mesh = tooth.getRootData()
#         vertices = mesh.vertices()
#         faces = mesh.faces()
#         faces = [i for face in faces for i in face]  # get flat faces
#         complete_vertices = np.array([vertices[i]
...
#     def get_short_root_vertices_faces(self, tooth_clID: int) -> tuple:
#         """Return the vertices and faces of a short root mesh"""
#         tooth = self.tp.getToothByID(tooth_clID) 
#         if tooth is None: # 
#             return None, None
#         mesh = tooth.getShortRootData()
#         vertices = mesh.vertices()
#         faces = mesh.faces()
#         faces = [i for face in faces for i in face]  # get flat faces
#         complete_vertices = np.array([vertices[i]
...
#     def get_pv_tooth_mesh(self, tooth_clID: int) -> pv.PolyData:
#         """Return a pyvista mesh of a tooth"""
#         vertices, faces = self.get_tooth_vertices_faces(tooth_clID)
#         if vertices is None or faces is None:
#             return None
#         pv_faces = np.array([[3, i, i+1, i+2]
#                             for i in range(0, len(faces), 3)]).flatten()
#         mesh = pv.PolyData(vertices, pv_faces)
#         return mesh

#     def convert_expanded_mesh_to_standard(self, vertices, faces):
#         """
...
#     def get_tooth_by_cl_id(self, tooth_clID: int):
#         """Return a tooth object by its clID"""
#         return self.tp.getToothByID(tooth_clID)
    
#     def get_treatment_plan(self):
#         return self.tp
    
#     def get_T2_stage(self)->int:
#         """Return the T2 stage number"""
#         return max([self.tp.GetJaw(jaw).GetT2() for jaw in JawType])
    
#     def has_tooth(self, tooth_clID: int) -> bool:
#         """Check if the case has a tooth with the given clID"""
#         return self.tp.getToothByID(tooth_clID) is not None # это неправильно работает...

#     def get_jaw_transform(self, jaw: JawType) -> ormco.PyRigidTransform:
#         """Return the relative transform for a given jawType ID"""
#         return self.tp.GetJaw(jaw).GetRelativeTransform(0)
