...
def show_points_in_pv(points, title="Points"):
    """
    ...
    plotter = pv.Plotter()
    n_teeth = points.shape[0]
    rng = np.random.default_rng(42)  # Seed for reproducibility

    for tooth_idx in range(n_teeth):
        tooth_points = points[tooth_idx]
        color = rng.uniform(0, 1, 3)  # Random RGB color
        point_cloud = pv.PolyData(tooth_points)
        plotter.add_mesh(point_cloud, color=color, point_size=10, render_points_as_spheres=True)
...
class OrthoCaseLoader:
    ...
    def get_landmarks_orgins(self):
        return case_landmark_grids_orgins(self.ortho_case)
...
class OrthoInferencePipeline:
    ...
    def point_cloud_to_jaw(self, points, mandibular_jaw_rt, maxillary_jaw_rt):
        ...
        """
        points_ = points.copy()
        mandibular_matrix = get_transform_matrix_from_ormco(mandibular_jaw_rt)#.copy()
        maxillary_matrix = get_transform_matrix_from_ormco(maxillary_jaw_rt)#.copy()
