...
- NOTE jaw_rt = mandibular_rt if tooth_id > 30 else maxillary_rt # TODO рефакторить используя isLower 
...
- теперь рефактор. суть - убрать json файл с трансформами и все трансформы включая трансформы челюсти делать на беке, см. 
    ...
    - нормально в init_predict, но проблема с инверсией x и y координат сохраняется в run_t2_predict при например 
        ...
        насколько это хорошо - вопрос. надо проверить на кейсе у которого координаты челюсти повернуты на 90° а не на 180. 
            - проверка показывает что просто инверсии координат в этом случае недостаточно - прокатывает если jaw просто повернута на 180 в плоскости xy, но не прокатывает в общем случае. Решение - транслировать в челюсть только зубы темплейта, а базового кейса - нет. 
