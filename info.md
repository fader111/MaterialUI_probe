...
1042950 - натурал класс 2 не все зубы

1071181 - натурал класс 1 не все зубы
...
DONE еще хочется сделать чтобы система координат контролов зубов была 
    привязана к зубу, а не к 
    ориентирована как глобальная. тогда может быть не понадобится арка...

TODO - сделать форк трансформ контролов и настроить толщину линий, либо попробовать использовать трансформ контролы из Three.js напрямую. 
что дальше????? показывать это как есть может вызвать вопросы из-за дыр и коллизий.  
    пробовать делать collision-space resolver. !!!!!!!
    но сначала - 
    вариант -  
    для этого придется где-то сохранять файл oas с измененными трансформами что есть жутчайшее гемор, т.к. я не знаю в каком виде это хранится в oas...
    
    вариант 2 - использовать редактированный t2 с помощью контролов в качестве темплейта. т.е. пробовать делать инференс используя не oas входные файлы а лендмарки скорректированного в t2 зуба.. для этого возможно придется доработать run_t2_predict введя в него точки и не вычисляя их из кейса, если они не None. выведу в консоль и посмотрю что за точки. а это и не точки вовсе, 
    у меня там трансформы. и вот надо получив их в предикте в качестве входного аргумента использовать в качестве данных темплейта - для этого: 
        point cloud уже есть - это base_case_points_t2 . к нему надо только применить новые трансформы - собственно взять их OrthoData, 
        они там окажутся после двиганий трансформ контролов. 
        пока пишем прогу с промежуточной идеей, что работаем с кейсом в котором T2 в принципе есть. 
        для работы с пустым кейсом понадобятся доработки.

    - сделал 2 работы предикта T2 = из файла темплейта и из вручную отредактированного T2 

вариант из темплейта сомнительный - причины регрессор учится на разностях и сильно влияет не просто конечное положение зубов а их разность
    - как идея - переучить регрессор не на разностях, а на позициях, посмотреть что выйдет
    - а выходит заметно лучше... оставим так

- наблюдение - если обучить модель лучше - стартовать с меньшего lr то лосс меньше, но она не корректирует грубые ошибки. меньше движения делает. 
параметры учебы для такой модели
batch_size = 128 
num_epochs = 1500 
lr = 0.0002
patience = 50
scheduler_factor = 0.92 
scheduler_patience = 20
итоговый loss val - 0.3 против 0.6 у менее обученной модели.

- DONE убрать стейджинг (модифицировать слайдер) в слайдере step={maxStage} вместо 1
- наметилась капитальная проблема на этапе как это сделать доступным для всех:
    cython не компилируется для линукса - нельзя сделать докер и сунуть это всё в корпоративную сеть. 
    может и к лучшему - переходим на oasdatabase скомпилированный без cython и делаем всё на stl файлах и json с настройками. 
    переходим в ветку docker_ready.
- сделал oasdatabase который умеет экспортировать stl файлы и json с параметрами сетапа. см.    
    E:\WebGLServer\orthoplatform и проверил его. 
- следующий шаг - надо передылвать api сервера, 
    - генерация мешей идет довольно быстро 
- затем переделка инференса под новый вариант.
